...
a.trigger(b,[a,this]);this.toggleInsertMultiRow()},uploadError:function(a,b,c){b=this.getObjectFromMessage(b);this.setUploading(!1);var d=this.fileMap[a.uniqueIdentifier];if(d&&b.errors){c=b.errors[0];if(!c)for(var e in b.errors){c=b.errors[e];break}d.find(this.options.templateProgress).remove();d.find(this.options.templateError).text(c);d.addClass("is-uploadError");delete this.fileMap[a.uniqueIdentifier];d.removeData("file")}else XF.defaultAjaxSuccessError(b,200,c.xhr),this.removeFileRow(d)},actionButtonClick:function(a){a.preventDefault();
var b=c(a.currentTarget);a=b.attr("data-action");var d=b.attr("data-type");b=b.closest(this.options.fileRow);switch(a){case "thumbnail":case "full":this.insertAttachment(b,a,d);break;case "delete":this.deleteAttachment(b,d);break;case "cancel":this.cancelUpload(b)}},checkboxClick:function(){var a=this.$filesContainer.find("input:checkbox").filter(":checked").length;c(this.options.selectActionButton).prop("disabled",a?!1:!0)},selectToggleClick:function(a){a.preventDefault();this.setSelectActionState(!this.$container.hasClass("is-selecting"));
